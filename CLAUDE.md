...
## Status (as of end of 2026-07-12)
Worklogs: 2026-07-07/08 (detail), 2026-07-10 (consolidated + engineering rules), **2026-07-11 (READ FIRST — final interface model)**, 2026-07-12 (latest).
- ALL shipped + confirmed live: v1.0 (title/badge, prototype footer gone), About page, mobile pass + collapsible mobile control drawer, Cloudflare Web Analytics beacon on all 4 pages, **data.html** downloads page (latest-only COGs; publisher INCLUDE_COGS=1 + 25 MiB per-file guard).
- Viewer also has: obs layer w/ dynamic dots + obs curves, sparkline popups, archive, verification (3 metrics incl. MDA8 + exceedances) + per-site badges, HMS smoke + clustered VIIRS fires, Class I/tribal/grid overlays, collapsible overlay panel, topo basemap, permalinks, find-a-place, Pacific-time UI, failure emails.
...
- Queue: accessibility pass (#35), smoke-day indicator (#37), Phase 2 GOES-R FDC FRP (Kamiak job, #25); monthly statistics (~Aug 1).
